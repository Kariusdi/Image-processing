...
def LocalHistEqualization(user_gamma):

    #clear all old pics first
    dir = 'dividepics/input'
    for file in os.scandir(dir):
        os.remove(file.path)
    dir = 'dividepics/output'
    for file in os.scandir(dir):
        os.remove(file.path)
    ...
    for img in range(len(blocks)):
        ...
        mean = float(np.mean(imgblock_in))
        print(mean)
        gamma = user_gamma
        ...
        gamma_corrected = ((imgblock_in / 255)**gamma) * 255
...
user_gamma = float(input('Please enter your gamma: '))
LocalHistEqualization(user_gamma)